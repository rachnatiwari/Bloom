...
const AwardToolTip = styled(({ className, ...props }: any) => (
  <Tooltip {...props} arrow classes={{ popper: className }} />
))(({ theme }) => ({
  [`& .${tooltipClasses.arrow}`]: {
...
const SubredditTooltip = styled(({ className, ...props }: any) => (
...
const SnoovatarTooltip = styled(({ className, ...props }: any) => (
  <Tooltip {...props} classes={{ popper: className }} />
))(({ theme }) => ({
  [`& .${tooltipClasses.tooltip}`]: {
    background:
...
const UserTooltip = styled(({ className, ...props }: any) => (
  <Tooltip {...props} classes={{ popper: className }} />
))(({ theme }) => ({
  [`& .${tooltipClasses.tooltip}`]: {
    background: "#fff",
...
function CardNav(props: any) {
  ...
  async function fetchUser() {
    redditApi.getUserAbout(props.author)
      .then((data) => {
        let raw_data = data.data;
        let user_info = {
  ...
  return (
    <div className="card-nav">
      <div style={{ width: "4%", marginTop: "0.4%" }}>
        <img
            src={subreddit.icon ? subreddit.icon : subredditIcon}
          alt="subreddit Icon"
            ...
            title={
              <div style={{ padding: "3%" }}>
                ...
                <div className="snoovatar_username">
                  {user.name}
                  {!user.snoovatar_img && (
                    <img src={premiumIcon} alt="premium user" style={{ maxWidth: "10%" }} />
                  )}
                </div>
